...
case class CompressTreeSolution(solutions: Seq[StageSolution]) {
  def getLatency: Int                           = solutions.length
  def getAreaCostInStage(stage: Int): Int       = solutions(stage).getAreaCost
...
case class Consideration(areaCost: Int = 0, reductionEfficiency: Double = 0, reductionRatio: Double = 1, bitReduction: Int = 0, heightReduction: Int = 1)

case class StageInfo(stageHeightDiff: String, wholeHeightDiff: String, nextBitHeapConfig: Seq[BitHeapConfigInfo[Int]], finalStage: Boolean)
