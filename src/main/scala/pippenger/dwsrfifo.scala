...
class dwsrfifo[T <: Data](dataType: HardType[T], depth: Int) extends Component {
  ...
  val io = new Bundle {
    val fromInput = slave Stream dataType()
  ...
  def empty: Bool = pushPtr === popPtr

  def full: Bool = pushPtr.msb =/= popPtr.msb & pushPtr(log2Up(depth) - 1 downto 0) === popPtr(log2Up(depth) - 1 downto 0)
...
object dwsrfifo extends App {
  ...
  //        }
  //        if (dut.io.fromInput.valid.toBoolean & dut.io.fromInput.ready.toBoolean) {
