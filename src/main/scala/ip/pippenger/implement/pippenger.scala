package ip.pippenger.implement
...
class pippenger[T <: Data](pType: HardType[T], pInit: => T, N: Int, W: Int, w: Int, latency: Int)
  ...
  shiftRegs.foreach(_.io.dataIn.assignDontCare())
  ...
  val fsm = new StateMachine {
    ...
    val stage1KNext = buffer(stage1K, stateRegs(0).latency)
    val stage1PNext = buffer(stage1P, stateRegs(0).latency)
    val stage1KValidNext = buffer(stage1KValid, stateRegs(0).latency, False)
    val adderOutputKNext = buffer(adderOutputK, stateRegs(0).latency)
    val adderOutputP = Vec(io.adderPort.map(_.s))
    val adderOutputValidNext = buffer(adderOutputValid, stateRegs(0).latency, Vec(False, 2))

    val kSame = buffer(Vec(stage1KValidNext & adderOutputValidNext(0) & stage1KNext(0) === adderOutputKNext(0) & stage1KNext(0) =/= 0,
      stage1KValidNext & adderOutputValidNext(1) & stage1KNext(1) === adderOutputKNext(1) & stage1KNext(1) =/= 0), mem(0).latency)
    val stage1KNextNext = buffer(stage1KNext, mem(0).latency)
    val stage1PNextNext = buffer(stage1PNext, mem(0).latency)
    val stage1KValidNextNext = buffer(stage1KValidNext, mem(0).latency, False)
    val adderOutputKNextNext = buffer(adderOutputKNext, mem(0).latency)
    val adderOutputPNext = buffer(adderOutputP, mem(0).latency)
    val adderOutputValidNextNext = buffer(adderOutputValidNext, mem(0).latency, Vec(False, 2))
    val stateRegsOutputNext = buffer(Vec(stateRegs.map(_.io.readData)), mem(0).latency)
    ...
    val stage2FirstCntLsb = buffer(stage2FirstCnt.lsb, maxLatency)
    val stage2FirstWaitValid = Reg(Bool()) init (False)

    val stateRegsReadAddress = UInt(log2Up(G) + w bits).assignDontCare()
    val stateRegsReadData = buffer(Mux(buffer(stateRegsReadAddress(w), stateRegs(0).latency), stateRegs(1).io.readData(0), stateRegs(0).io.readData(0)), maxLatency - stateRegs(0).latency)
    val memReadAddress = UInt(log2Up(G) + w bits).assignDontCare()
    val memReadEn = False
    val memReadDataBeforeBuffer = Mux(buffer(memReadAddress(w), mem(0).latency), mem(1).io.port(0).readData, mem(0).io.port(0).readData)
    val memReadData = buffer(memReadDataBeforeBuffer, maxLatency - mem(0).latency)
    ...
    val stage2WorkGCntLsb = buffer(stage2WorkGCnt.lsb, maxLatency)
    val stage2WorkWaitValid = Reg(Bool()) init (False)

    val sDataReg1 = buffer(io.adderPort(0).s, maxLatency - 1)
    ...
    val stage3AddValidBuffer = buffer(stage3AddValid, mem(0).latency)
    ...
    val sDataReg3 = buffer(io.adderPort(0).s, mem(0).latency)
    ...
    stage3.whenIsActive {
      ...
      io.adderPort(0).a := Mux(stage3AddValidBuffer, memReadDataBeforeBuffer, sDataReg3)
  ...
  def buffer[T <: Data](input: T, latency: Int, init: T = null.asInstanceOf[T]): T = {
    var output = input
    for (i <- 0 until latency) {
      if (init == null) {
        if (i != latency - 1) {
          output = RegNext(output)
        } else {
          output = RegNext(output)
        }
      } else {
        if (i != latency - 1) {
          output = RegNext(output) init (init)
        } else {
          output = RegNext(output) init (init)
        }
      }
...
object pippenger extends App {
  //org.datenlord.VivadoImpl(new pippenger(UInt(377 bits), U(0, 377 bits), 1 << 26, 253, 10, 300))
