...
object PagSuiteParser {
  ...
  def build(constants: Seq[BigInt], widthIn:Int) = (dataIn: UInt) => {
    ...
    def getOperand(operand: Operand) = signalMap(operand.stage, operand.fundamental.abs) << operand.shift
    ...
    nodes.foreach { node =>
      ...
      node.nodeType match {
        ...
        case TERNARY =>
          ...
          val ret = config.asNode(Seq(op0, regularOp1, regularOp2)).head
          signalMap += node.outPosition -> ret
          logger.info(s"+${ret.getBitsWidth}")
          cost += ret.getBitsWidth
      }
...
case class PagMcm(constants: Seq[BigInt], widthIn:Int) extends Component {
  val impl = PagSuiteParser.build(constants,widthIn)
  val dataIn = in UInt (widthIn bits)
  val ret = impl(dataIn)
  val dataOut = out Vec (ret.values.toSeq)
}
...
object PagMcm {
  ...
  def main(args: Array[String]): Unit = {
    println(PagSuiteParser.run(Seq(BigInt(135), BigInt(257)), 12))
    VivadoSynth(PagMcm(Seq(BigInt(135), BigInt(257)), 12))
  }
}
