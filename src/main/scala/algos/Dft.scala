...
object Dft {
  ...
  def genericDFTMatrix[T: ClassTag](N: Int, inverse: Boolean, omega: T)(implicit field: Field[T]) = {
    val base = if (inverse) omega else field.one / omega
  ...
  // for complex field
  def omega(N: Int) = exp((2 * Pi / N) * i)

  def diagT(N: Int, n: Int) =
    Matrices.diagonal(Seq.tabulate(N / n, n) { (i, j) =>
      Seq.fill(i * j)(Complex(1, 0) / omega(N)).product
    }.flatten)

  def diagC(N: Int, l: Int, radix: Int) = {
    val pow: Int => Int = powR(radix, _)
    val t = logR(N, radix)
    val part0 = Matrices.stridePermutation[Complex](N, pow(t - l - 1))
    val T = diagT(pow(t - l), pow(t - l - 1))
    val part1 = Matrices.kronecker(T, pow(l))
    val part2 = Matrices.stridePermutation[Complex](N, pow(l + 1))
    part0 * part1 * part2
  }

  def dftMatrix(N: Int, inverse: Boolean) = genericDFTMatrix(N, inverse, omega(N))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val golden = dftMatrix(8, false) * data
    val pease = peaseFftMatrix(8, 2, false) * data
