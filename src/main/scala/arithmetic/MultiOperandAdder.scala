...
case class MultiOperandAdderConfig(infos: Seq[ArithInfo]) extends TransformDfg {
  ...
  val cpaConfig  = CpaConfig(widthOutMerge, if (hasNegative) TernaryAdder else BinaryAdder, if (hasNegative) 1 else 0) // final two rows - compensation
  val cpaLatency = cpaConfig.latency

  override val name      = "MultiOperandAdder"
  ...
  logger.info(
    s"\n----latency report of bit heap compressor----" +
      ...
      s"\n\tCSA latency: ${latency - cpaLatency - 2}, CPA latency: $cpaLatency"
  )

  override def implH = MultiOperandAdder(this)
}

case class MultiOperandAdder(config: MultiOperandAdderConfig) extends TransformModule[UInt, UInt] {
  ...
  if (!hasNegative) {
    ...
    val cpaResult = cpaConfig.asFunc(Seq(rows.head, rows.last))
    dataOut.fragment.head := (cpaResult.head @@ U(0, heapOut.weightLows.head bits)).d(1)
  } else {
    ...
    val cpaResult = cpaConfig.asFunc(Seq(rows.head, rows.last, compensation >> retMerge.weightLows.head))
    dataOut.fragment.head := (cpaResult.head @@ U(0, retMerge.weightLows.head bits)).d(1)
  }
