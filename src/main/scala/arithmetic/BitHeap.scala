...
/** Storing information of a bit matrix(heap), while providing util methods, making operations on bit matrix easier
  ...
  * @example
  *   bitHeap(m)(n) is the (n+1)-th bit of (m+1)-th column
  * @param weightLow
...
case class BitHeap[T](bitHeap: ArrayBuffer[ArrayBuffer[T]], weightLow: Int) {
  ...
  def takeLow(widthTake: Int): BitHeap[T] = BitHeap.getHeapFromTable(bitHeap.take(widthTake - weightLow), widthTake)
  ...
  /** get the exact(rather than maximum) efficiency of a compressor applied on current bit heap
    ...
    */
  def getExactEfficiency(
      compressor: Compressor[_],
      width: Int,
      columnIndex: Int
  ): Double = {
  ...
  /** get the most efficient compressor for current bit heap and do compression with it
    ...
    * @tparam T0
    *   the type is T0 rather than T, as we may want to use Compressors[Bool] on BitHeap[Int] for simulation
    * @return
    *   new bit heap generated by the compressor, and the LUT cost
    */
  def compressOneTime[T0](
      compressors: Seq[Compressor[T0]],
      finalStage: Boolean
  ): (BitHeap[T], Int, String) = {
    ...
    val newHeap = mark match {
      case t: Int =>
        fakeImpl(bestCompressor, bestWidth, columnIndex) // when T is Int, T0 != T
  ...
  def compressOneStage[T0](
    ...
    while (!isEmpty) {
      val (heap, cost, compressorName) =
        compressOneTime(compressors, finalStage)
      results += heap
      stageCost += cost
    ...
    val nextStage = mark match {
      ...
      case _ =>
        results
          .asInstanceOf[Seq[BitHeap[T0]]]
          .reduce(_ + _)
          .d(pipeline)
          .asInstanceOf[BitHeap[T]] // when T is Bool, T0 = T
    }
  ...
  def compressAll[T0](
    ...
    while (current.height > 2 && latency < 100) {
      ...
      val (heap, cost) =
        current.compressOneStage(candidates, pipeline, finalStage)
      current = heap
      allCost += cost
      latency += 1
    }
  ...
  override def toString = {
    ...
    val dotDiagram = heights
      .padTo(finalWidth, 0)
      .map(columnHeight => Seq.fill(columnHeight)("\u25A0").padTo(this.height, " "))
      .reverse
      .transpose
      .map(_.mkString(" "))
      .mkString("\n")
...
object BitHeap {
  ...
  /** build a BitHeap[Int] with a specified shape for simualtion
    */
  def getFakeHeapFromHeights(
      heights: Seq[Int],
      weightLow: Int = 0
  ): BitHeap[Int] =
    BitHeap[Int](
      ArrayBuffer(heights: _*).map(i => ArrayBuffer.fill(i)(0)),
      weightLow
    )
