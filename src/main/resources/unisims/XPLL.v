...
`ifndef XIL_XECLIB
  always @ (trig_attr) begin
    ...
    if ((attr_test == 1'b1) ||
      ...
      $display("Error: [Unisim %s-149] VCOCLK_FREQ_MAX attribute is set to %f.  Legal values for this attribute are 4320.000 to 4320.000. Instance: %m", MODULE_NAME, VCOCLK_FREQ_MAX_REG);
      attr_err = 1'b1;
    end
    
    if ((attr_test == 1'b1) ||
        (VCOCLK_FREQ_MIN_REG < 2160.000 || VCOCLK_FREQ_MIN_REG > 2160.000)) begin
      $display("Error: [Unisim %s-150] VCOCLK_FREQ_MIN attribute is set to %f.  Legal values for this attribute are 2160.000 to 2160.000. Instance: %m", MODULE_NAME, VCOCLK_FREQ_MIN_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
      ...
      $display("Error: [Unisim %s-151] XPLL_CONNECT_TO_NOCMC attribute is set to %s.  Legal values for this attribute are NONE, DDR or LP4. Instance: %m", MODULE_NAME, XPLL_CONNECT_TO_NOCMC_REG);
      attr_err = 1'b1;
    end
...
//            if (valid_daddr) dr_sram[DADDR_in] <= DI_in;
 ...
 always  @(negedge clkvco or posedge rst_in_o)
  ...
  always @(posedge clkfb2_deskew_in_dly or posedge rst_int)
    ...
    always @(clk0_div or clk1_div or clk2_div or clk3_div or 
             fb_d1_det_delay or fb_d2_det_delay or period_vco_rl or 
             d1_delay_found or d2_delay_found or rst_in_o or
...
`ifdef XIL_TIMING
    ...
    $width (negedge CLKOUTPHYEN, 0:0:0, 0, notifier);
    ...
    $width (posedge CLKOUTPHYEN, 0:0:0, 0, notifier);
