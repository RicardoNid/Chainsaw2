...
class CordicTest extends AnyFlatSpec {
  ...
  def test(alg: AlgebraicMode, rot: RotationMode) = {
    val config = CordicConfig(alg, rot, iteration = testIteration, fraction = testFraction)
  ...
  it should s"work for CIRCULAR   + VECTOR mode" in test(CIRCULAR, VECTORING) // get phase and abs of a complex number
