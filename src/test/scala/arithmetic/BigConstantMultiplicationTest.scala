...
class BigConstantMultiplicationTest extends AnyFlatSpec {
  ...
  def config0 = BigConstantMultiplicationConfig(MPrime, dataWidth + 1, FULL)
  ...
  def config1 = BigConstantMultiplicationByCompressorTreeConfig(MPrime, widthIn = dataWidth + 1, HALFHIGH, widthTake = dataWidth + 4, useCsd = true)
  ...
  def config2 = BigConstantMultiplicationByCompressorTreeConfig(baseModulus, widthIn = dataWidth + 1, HALFLOW, widthTake = dataWidth + 2, useCsd = true)
