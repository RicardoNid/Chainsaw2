...
import org.datenlord.xilinx.{VivadoReport, VivadoUtil}
...
class GpcsTest extends AnyFlatSpec {
  ...
  //  Random.setSeed(42)
  val defaultCompressorPerfGraphPath = "./CompressorPerfReport"
  ...
  case class CompressorPerfReportGraph(perfReportGraphPath: String = defaultCompressorPerfGraphPath) {
    val elements                = Set[(Compressor, Int, VivadoReport)]()
    val fmaxGraphDataInRowAdder = Set[(String, (String, ArrayBuffer[Int]), (String, ArrayBuffer[Double]))]()
    val areaGraphDataInRowAdder = Set[(String, (String, ArrayBuffer[Int]), (String, ArrayBuffer[Int]))]()
    val fmaxGraphDataInGPC      = Set[((String, ArrayBuffer[String]), (String, ArrayBuffer[Double]))]()
    val areaGraphDataInGPC      = Set[((String, ArrayBuffer[String]), (String, ArrayBuffer[Int]))]()
  ...
  def testCompressorFuncOnce(compressor: Compressor, width: Int, testCount: Int = 1000, debug: Boolean = false, name: String = "funcTest_temp"): Unit = {
    ...
    SimConfig
      ...
      .doSim { dut =>
        val data = Seq.fill(testCount)(Random.nextBigInt(inputShape.sum))
        data.foreach { value =>
          dut.dataIn #= value
          sleep(1)
  ...
  def compressorFuncTest: Unit = {
    ...
    TESTCASE.foreach { case (compressor, widths) =>
      it should s"work correctly on ${compressor.name}" in widths.foreach(width => testCompressorPerfOnce(compressor, width))
    }
  }

  def compressorPerfTest(genPerGraph: Boolean = true, perfReportGraphPath: String = defaultCompressorPerfGraphPath): Unit = {
    val perfGraph = if (genPerGraph) CompressorPerfReportGraph(perfReportGraphPath) else null
