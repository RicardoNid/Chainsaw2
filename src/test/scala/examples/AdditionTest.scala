...
class AdditionTest extends AnyFlatSpec {
  ...
  def testBigAddersOfWidths(widths: Seq[Int]): Unit = {
    val reports: Seq[(Int, VivadoReport)] = widths.map(width => width -> VivadoSynth(PipelinedBigAdder(PipelinedBigAdderConfig(width, 63)), name = s"additionTest_$width"))
    reports.foreach { report =>
      println(s"${report._1} -> efficiency of LUT = ${report._2.LUT / report._1.toDouble}, CARRY8 = ${report._2.CARRY8 / report._1.toDouble}, FMax = ${report._2.Frequency / 1e6}MHz ")
    }
  }
  ...
  "this test" should "get cost and fmax of adders of different widths" in testAddersOfWidths(Seq(256 - 1, 512 - 1, 1024 - 1))
